...
import { AdminDashboardStore } from './stores/admin-dashboard.store';
...
export class DashboardComponent {
  ...
  barHeight(vol: number): number {
    return Math.max(4, (vol / this.store.maxVolume()) * 100);
  }
  ...
  statusPercent(count: number): number {
    const total = this.store.podStatusBreakdown().reduce((a, b) => a + b.count, 0);
